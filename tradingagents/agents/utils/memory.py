...
from tradingagents.agents.utils.rating import RATING_REVIEW, parse_rating
...
class TradingMemoryLog:
        ...
        if self._log_path.exists():
            ...
            for line in raw.splitlines():
                if line.startswith(f"[{trade_date} | {ticker} |") and line.endswith("| pending]"):
                    return
        rating = parse_rating(final_trade_decision, default=RATING_REVIEW)
    ...
    def get_pending_entries(self) -> list[dict]:
        ...
        return [e for e in self.load_entries()
                if e.get("pending") and e.get("rating") != RATING_REVIEW]
