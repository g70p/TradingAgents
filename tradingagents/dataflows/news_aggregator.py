...
def _parse_rss_titles(xml_text: str, limit: int = 15, *, start_date=None, end_date=None) -> list[str]:
...
def _source_google_news(query: str, lang: str = "en", limit: int = 8, *, start_date: str | None = None, end_date: str | None = None) -> str | None:
    """Google News RSS."""
    ...
    if not xml:
        return None
    titles = _parse_rss_titles(xml, limit, start_date=start_date, end_date=end_date)
    return "\n".join(f"- {t}" for t in titles) if titles else None


def _source_euronext(ticker: str, limit: int = 5, *, start_date: str | None = None, end_date: str | None = None) -> str | None:
    ...
    return _source_google_news(query, limit=limit, start_date=start_date, end_date=end_date)


def _source_jornal_negocios(ticker: str, limit: int = 5, *, start_date: str | None = None, end_date: str | None = None) -> str | None:
...
def _source_investing(ticker: str, limit: int = 5, *, start_date: str | None = None, end_date: str | None = None) -> str | None:
    ...
    for feed in feeds:
        xml = _fetch_rss(feed)
        if xml:
            all_titles.extend(_parse_rss_titles(xml, limit=3, start_date=start_date, end_date=end_date))
...
def _source_cnbc(limit: int = 5, *, start_date: str | None = None, end_date: str | None = None) -> str | None:
    ...
    for feed in feeds:
        ...
        if xml:
            titles = _parse_rss_titles(xml, limit=3, start_date=start_date, end_date=end_date)
...
def _source_marketwatch(limit: int = 5, *, start_date: str | None = None, end_date: str | None = None) -> str | None:
...
def fetch_news_multi_source(ticker: str, limit_per_source: int = 6, *, start_date: str | None = None, end_date: str | None = None) -> str:
    ...
    ticker.endswith("-USD")
    any(ticker.endswith(s) for s in (".LS", ".PA", ".AS", ".BR", ".LX"))
    ...
    if is_pt:
        pt = _source_google_news(f"{base} cotação", lang="pt", limit=limit_per_source, start_date=start_date, end_date=end_date)
        if pt:
            parts.append(f"### 🇵🇹 Google News PT: \"{base}\"\n{pt}")
            total_news += pt.count("\n") + 1

    en = _source_google_news(f"{ticker} stock price analysis", limit=limit_per_source, start_date=start_date, end_date=end_date)
    if en:
        parts.append(f"### 🌍 Google News EN: \"{ticker}\"\n{en}")
        total_news += en.count("\n") + 1

    # 2. Euronext (para stocks europeus)
    euronext = _source_euronext(ticker, limit=limit_per_source, start_date=start_date, end_date=end_date)
    if euronext:
        parts.append(f"### 🏛️ Euronext: {ticker}\n{euronext}")
        total_news += euronext.count("\n") + 1

    # 3. Jornal de Negócios (para PT)
    negocios = _source_jornal_negocios(ticker, limit=limit_per_source, start_date=start_date, end_date=end_date)
    if negocios:
        parts.append(f"### 📰 Jornal de Negócios: {ticker}\n{negocios}")
        total_news += negocios.count("\n") + 1

    # 4. Investing.com (global)
    investing = _source_investing(ticker, limit=limit_per_source, start_date=start_date, end_date=end_date)
    if investing:
        parts.append(f"### 📈 Investing.com: mercados\n{investing}")
        total_news += investing.count("\n") + 1

    # 5. CNBC (internacional)
    cnbc = _source_cnbc(limit=limit_per_source, start_date=start_date, end_date=end_date)
    if cnbc:
        parts.append(f"### 🇺🇸 CNBC: notícias de mercados\n{cnbc}")
        total_news += cnbc.count("\n") + 1

    # 6. MarketWatch (EUA)
    mw = _source_marketwatch(limit=limit_per_source, start_date=start_date, end_date=end_date)
    if mw:
