...
class TradingAgentsGraph:
    ...
    def _fetch_returns(
        ...
        Returns four None values when the full holding window is unavailable.
        """
    ...
    def collect_data(self, ticker: str, trade_date: str, asset_type: str = "stock") -> dict:
        ...
        historical = requested < datetime.now().date()
        start_date = (requested - timedelta(days=7)).isoformat()
        data = {}
        errors = []

        def collect(key, function):
            try:
                value = function()
                if not value or str(value).startswith(("⚠", "Error", "(indisponível")):
        ...
        collect("market_session", lambda: get_market_context_for_state(ticker, asset_type, trade_date))
        if historical:
            data["instrument_context"] = f"Símbolo pedido: {ticker}; identidade atual não usada como prova histórica."
        else:
            collect("instrument_context", lambda: self.resolve_instrument_context(ticker, asset_type))
        collect("ticker_news", lambda: route_to_vendor("get_news", ticker, start_date, trade_date))
        collect("global_news", lambda: route_to_vendor("get_global_news", trade_date))
        collect("market_snapshot", lambda: route_to_vendor("get_stock_data", ticker, start_date, trade_date))
        collect("indicators", lambda: route_to_vendor("get_indicators", ticker, "rsi", trade_date, 30))
        collect("macro", lambda: route_to_vendor("get_macro_indicators", "DFF", trade_date, 365))
        for key, function in {
            "insider_tx": lambda: route_to_vendor("get_insider_transactions", ticker),
            "prediction_markets": lambda: route_to_vendor("get_prediction_markets", ticker),
            "crypto_onchain": lambda: get_crypto_onchain_summary(ticker) if asset_type == "crypto" else "Não aplicável",
        }.items():
            if historical:
                data[key] = "Indisponível: fonte atual sem arquivo histórico verificável."
                errors.append(f"{key}: no historical snapshot")
            else:
                collect(key, function)
        for tool in ("get_fundamentals", "get_balance_sheet", "get_cashflow", "get_income_statement"):
            key = tool.removeprefix("get_")
            if asset_type != "stock":
                data[key] = "Não aplicável"
            elif historical:
                data[key] = "Indisponível: datas de publicação/vintage não verificadas; período contabilístico não basta."
                errors.append(f"{key}: no verified publication vintage")
            else:
                collect(key, lambda tool=tool: route_to_vendor(tool, ticker, curr_date=trade_date))
        data["collection_errors"] = " | ".join(errors)
    ...
    def _run_graph(self, company_name, trade_date, asset_type: str = "stock",
        ...
        instrument_context = (f"Símbolo pedido: {company_name}; identidade histórica não verificada."
                              if self._memory_as_of(trade_date) else
        ...
        if asset_type == "crypto" and self._memory_as_of(trade_date) is None:
        ...
        market_context = get_market_context_for_state(company_name, asset_type, trade_date)
        if market_context:
